...
PV.removeProxy = function removeProxy(proxyId, callback) {
...
PV.initSession = function initSession(callback) {
...
PV._saveServerProxyInfo = function _saveServerProxyInfo(callback) {
...
PV.render = function render(width, height, callback) {
...
PV.resetViewport = function resetViewport(callback) {
...
PV.colorProxy = function colorProxy(displayProps, callback) {
...
PV.setPalette = function setPalette(paletteName, callback) {
    ...
    PV.session.call('pv.color.manager.select.preset', paletteOptions).then(function (result) {
        ...
        if (callback)
            callback(null, { success: true }); // Doesn't seem like this needs to block, but \bad if a new layer became last layer before completion
    }, callback);
};
PV.setOpacity = function setOpacity(opacity, callback) {
...
PV.addFilter = function addFilter(filterName, settings, callback) {
...
PV.showFirstFrame = function showFirstFrame(callback) {
    //    console.log('** Starting showLastFrameSyncable');
    PV.alterVideo(['first'], callback);
};
PV.showLastFrame = function showLastFrame(callback) {
    //    console.log('** Starting showLastFrameSyncable');
    PV.alterVideo(['last'], callback);
};
PV.playStopEnd = function playStopEnd(callback) {
...
PV.playRepeat = function playRepeat(callback) {
...
PV.rescale = function rescale(callback) {
...
PV.updateCamera = function (opts, callback) {
